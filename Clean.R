...
UK_Education_clean <- UK_Education %>%
  select('...2', 'number...13')%>%
  rename('LAD code' = '...2', 'Batchalors_degree_or_higher' = 'number...13')

#4) Clean UK_energy data for 2024
UK_energy_2024_clean <- UK_energy_2024 %>%
  select('Local Authority Code [note 1]', 'Total')%>%
  rename('LAD code' = 'Local Authority Code [note 1]', 'Total_Generation_Output_2024' = 'Total')

#5) Clean UK_energy data for 2014
UK_energy_2014_clean <- UK_energy_2014 %>%
  select('Local Authority Code [note 1]', 'Total')%>%
  rename('LAD code' = 'Local Authority Code [note 1]', 'Total_Generation_Output_2014' = 'Total') 
...
UK_Education_clean <- UK_Education_clean %>%
  filter(!is.na(`LAD code`) & !is.na(`Batchalors degree or higher`))
